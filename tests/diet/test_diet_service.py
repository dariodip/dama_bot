...
def test_get_meals_by_day(service):
    meal_day = service.get_meals_by_day("Example", date(2026, 8, 10))
    assert "latte" in " ".join(meal_day.colazione.food)


def test_get_meals_by_day_and_meal_type(service):
    meal = service.get_meals_by_day_and_meal_type("Example", date(2026, 8, 10), MealType.COLAZIONE)
    assert "latte" in " ".join(meal.food)
