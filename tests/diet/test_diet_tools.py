...
async def test_diet_tool_get_meals_by_day(registry, service_mock):
    ...
    expected_meals = d.MealDay(
        colazione=d.Meal(
            ...
            food=["1 porzione di pancakes", "30 g di composta di frutta"]
        ),
        spuntino=d.Meal(
            type=d.MealType.SPUNTINO,
            food=["1 mela", "30 g di mandorle"]
        ),
        pranzo=d.Meal(
        ...
        merenda=d.Meal(
            ...
            food=["1 yogurt greco", "1 kaki"]
        ),
        cena=d.Meal(
            type=d.MealType.CENA,
            food=["250 g (2 fette) di scaloppine di maiale (arista solo con farina)", "lattuga e carote all’insalata", "1 cucchiaio di olio", "1/2 avocado", "130 g di pane"]
        )
    )
